...
class Product(models.Model):
    ...
    def _search_product_quantity(self, operator, value, field):
        ...
        for product in self.with_context(prefetch_fields=False).sudo().search([], order='id'):
    ...
    def _compute_quantities_dict(self, lot_id, owner_id, package_id, from_date=False, to_date=False):
        ...
        second_quants_res = {product.id: (quantity, secondary_quantity) for product, quantity, secondary_quantity in Quant._read_group(domain_quant, ['product_id'], ['quantity:sum', 'secondary_quantity:sum'])}
        if dates_in_the_past:
